...
class color_controller():
	...
	def blob_callback(self, blobs):
		...
		for blob in blobs.blobs:
			...
			self.colors[self.count] = color_model(blob, self.camera_info, self.parent_frame, self.depth_image, self.cam_model, self.listener, self.broadcaster)
			
			#Make sure this blob isn't shitty.
			if self.colors[self.count].validate():

				#Publish to tf if master wishes it upon Dobby the programming elf.
				#if self.publish_tf and not already_published:
				self.colors[self.count].publish()

				#3d blobs that are in this blobs3d list are published to the /blobs_3d topic. 
				blobs3d.blobs.append(self.colors[self.count].toBlob3d())
			self.count+=1
