...
FSM_STATES = Enumerate("FSM_WAIT_FOR_MAP FSM_LOCALIZE FSM_NAVIGATE FSM_SEARCH FSM_OPTIMIZE FSM_APPROACH_OBJ FSM_FINAL_APPROACH FSM_STABLE FSM_SPIN FSM_MOVE_TO_BASE FSM_WAIT_FOR_ACTION")
    ...
    def blob_callback(self, blobs_3d):
        ...
        if(len(blobs_3d.blobs)):
            for blob in blobs_3d.blobs:
                if (blob.center.y>=0.000 and blob.center.z <= 3.0): # if blob is closer than 3m and below sensor level
    ...
    def speed_callback(self):
        ...
        # =================>
            ...
            if (self.tracked_blob.linear.z > self.max_z or self.tracked_blob.linear.z < self.min_z):
                ...
                if (self.angError>=0 and self.RotDerivator<=0) or (self.RotDerivator>=0 and self.angError<=0):
            ...
            if (self.angStable): # if stable position and orientation have been achieved
                ...
                if (self.tracked_blob.linear.z > 1.0): # if object is cloaser than 1 m
        ...
        # ===============>
            ...
            if (rospy.Time.now() < self.correction_start + rospy.Duration(2)):
                ...
                self.moveDuration = rospy.Duration((self.tracked_blob.linear.z - 0.15)/0.1)
                rospy.loginfo((self.tracked_blob.linear.z - 0.15)/0.1)
        ...
        # =============>
            ...
            if (self.arm_state.data == 'AlignCamera' or self.arm_state.data =='AlignBin'): # if arm signifies that object has been dropped
        ...
        # =============>
            ...
            # Next state transitions
            if ((self.arm_state.data == 'LocateBin') or (self.arm_state.data == 'SearchObj')):
                self.state = FSM_STATES.FSM_SPIN
            elif (self.arm_state.data == 'Dropped'): # if arm signifies that object has been dropped
...
if __name__ == '__main__':
    ...
    KdRot = rospy.get_param('arm_bot_base/KdRot', 0.0)
