...
class arm_bot_base():
    ...
    def __init__(self, KpRot, KdRot, tol_z, tol_ang, maxRotSpeed, max_z, min_z):
        ...
        # Publications
        self.speed_pub = rospy.Publisher('/mobile_base/commands/velocity', Twist) #/navigation_velocity_smoother/raw_cmd_vel # /mobile_base/commands/velocity
    ...
    def blob_callback(self, blobs_3d):
        ...
        if(len(blobs_3d.blobs)):
            for blob in blobs_3d.blobs:
                if (blob.center.y>=0.000 and blob.center.z <= 0.003): # if blob is closer than 3m and below sensor level
    ...
    def speed_callback(self):
        ...
        # ===============>
            ...
            if (rospy.Time.now() < self.correction_start + rospy.Duration(2)):
                ...
                self.moveDuration = rospy.Duration((self.tracked_blob.linear.z - 0.0004)/0.0001)
                rospy.loginfo((self.tracked_blob.linear.z - 0.0004)/0.0001)
