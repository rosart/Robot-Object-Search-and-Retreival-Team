...
FSM_STATES = Enumerate("FSM_LOCALIZE FSM_NAVIGATE FSM_SEARCH FSM_OPTIMIZE FSM_STABLE FSM_SPIN")
    ...
    def speed_callback(self):
        ...
        # =================>
            ...
            if (self.tracked_blob.linear.z > self.max_z or self.tracked_blob.linear.z < self.min_z):
                ...
                # Debug messages
                rospy.loginfo("zError: %f, pLinError: %f, dLinError: %f" %(self.zError, self.pLinError, self.dLinError))    
                rospy.loginfo("angError: %f, pRotError: %f, dRotError: %f" %(self.angError, self.pRotError, self.dRotError))
            
    
            # Next state transitions
            if (self.zStable and self.angStable): # if stable position and orientation have been achieved
