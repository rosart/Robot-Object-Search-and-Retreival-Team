...
FSM_STATES = Enumerate("FSM_LOCALIZE FSM_NAVIGATE FSM_SEARCH FSM_OPTIMIZE FSM_APPROACH_OBJ FSM_STABLE FSM_SPIN FSM_MOVE_TO_BASE")
...
class arm_bot_base():
    ...
    def speed_callback(self):
        ...
        if self.state == FSM_STATES.FSM_LOCALIZE:
            ...
            if(self.blobs_detected and (self.tracked_blob.linear.y >= 0)): # if a blob is seen
        ...
        # =============>
            ...
            if(self.blobs_detected and (self.tracked_blob.linear.y >= 0)): # and (self.tracked_blob.linear.z < 3.0): # if a blob is seen (and closer than 3 meters)
        ...
        # ===============>       
            ...
            # Next state transitions
            if (self.blobs_detected and (self.tracked_blob.linear.y >= 0)):
                self.state = FSM_STATES.FSM_OPTIMIZE
            else:
        ...
        # =================>
            ...
            elif(self.blobs_detected and (self.tracked_blob.linear.y >= 0)):    # else if (while) blobs are being detected
                    self.state = FSM_STATES.FSM_OPTIMIZE # loop in same state
            else:
                self.state = FSM_STATES.FSM_OPTIMIZE # else move to nav state
        ...
        # =============>
            ...
            if ( self.tracked_blob.linear.z > 2.2):
                self.obj_pose.target_pose.pose.position.x = self.tracked_blob.linear.z - 2.0 # set goal to (distance - 2m)
            ...
            elif (self.tracked_blob.linear.z > 1.2):
            ...
            # else if distance is less than 1m 
                ...
                self.obj_pose.target_pose.pose.position.x = self.tracked_blob.linear.z - 0.4 # set goal to final target 
                self._move_base.send_goal(self.obj_pose) # send goal
        ...
        # =============>
            ...
            # Next state transitions
            # -Until arm sees object-! (Current content is temprorary)
...
if __name__ == '__main__':
    ...
    max_z = rospy.get_param('arm_bot_base/max_z', 6.0)
